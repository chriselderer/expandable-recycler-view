...
public class ExampleTwoActivity extends AppCompatActivity
{
    ...
    ExpandableRecyclerViewAdapterImplementationTwo adapter;
    ...
    private void setUpRecyclerView() {
        // Set up adapter
        adapter = new ExpandableRecyclerViewAdapterImplementationTwo(this);

        // Set up decorations
        ExpandableDecorationImplementationTwo decoration = new ExpandableDecorationImplementationTwo(this);
...
// region LOAD DATA
    ...
    public void loadCats() {
        ...
        // The first time we load cats we will show an error.
        if(catErrorOccured == false)
        {
            final Handler handler = new Handler();
            handler.postDelayed(new Runnable()
            {
                ...
                public void run()
                {
                    adapter.setExpansionStateError(1);
                }
            }, 1800);

            catErrorOccured = true;
            return;
        }
